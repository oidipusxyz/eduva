...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex fixed top-0 left-0 h-screen w-[72px] lg:w-[220px] bg-bg-surface border-r border-border-default flex-col py-4 gap-1 z-50 transition-all">
      {/* Logo */}
      <div className="flex items-center gap-3 px-3 mb-4">
        ...
        <span className="hidden lg:block text-[15px] font-semibold text-text-primary">Eduva</span>
      </div>

      <nav className="flex flex-col gap-0.5 flex-1 px-2 overflow-y-auto">
        {navItems.map((item, i) => {
          if (item.type === 'divider') {
            return <div key={i} className="mx-2 border-t border-border-muted my-1.5" />
          }
